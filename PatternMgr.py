...
class PatternMgr:
    ...
    def chineseFormat(self,sentence):
        # return ' '.join(jieba.cut(sentence))
        result = []
        word = ""
        for c in sentence:
            if self.isChinese(c) or c in "*_":
                if word:
                    result.append(word)
                    word = ''
                result.append(c)
    ...
    def add(self, data, template):
        ...
        # changed by Johnny4Fun
        pattern = self.chineseFormat(pattern)
        that = self.chineseFormat(that)
        topic = self.chineseFormat(topic)
    ...
    def match(self, pattern, that, topic):
        ...
        topicInput = self.chineseFormat(topicInput)
    ...
    def star(self, starType, pattern, that, topic, index):
        ...
        if foundTheRightStar:
            ...
            if starType == 'star': return ''.join(input_.split()[start:end+1])
